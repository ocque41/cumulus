...
`NEXT_PUBLIC_SITE_URL` is deliberately browser-visible. Every other notification variable is server-only. Synthetic examples such as `reader@example.com` are safe; sanitized-looking copies of live data are not. Outlook is outside the Cumulus system and remains untouched.
