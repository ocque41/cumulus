...
Document what actually happened: candidate commit, approvals, provider verification, migration result, production deployment, checks, and any remaining uncertainty. For the current release, the production Resend webhook and signing-secret variable are configured and an unsigned live probe is rejected. Notification publication remains intentionally unavailable until the truthful postal address is configured and a controlled lifecycle proves sign-in, receipt, unsubscribe, and later suppression.
