...
export function renderPostNotification(input: {
  ...
  return {
    ...
    html: [
      `<!doctype html><html lang="en" dir="ltr"><head><meta charset="utf-8"><title>${escapeHtml(title)}</title></head>`,
      ...
      '<p style="color:#ff5a00;margin:0 0 20px">NEW LOG</p>',
      ...
      `<p><a href="${escapeHtml(input.postUrl)}" style="color:#ff6a00">Read ${escapeHtml(title)}</a></p>`,
