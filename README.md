...
For automatic delivery, store the Production `NOTIFICATION_PUBLISH_SECRET` as a masked GitHub Actions secret with the same name. After Vercel reports a successful Production deployment, the repository workflow reconciles every published slug outside the fixed pre-automation baseline, validates the canonical endpoint, and requests the idempotent Resend Broadcast. Reconciliation prevents a batched or skipped deployment from losing a notification; already-sent slugs remain safe because publication is idempotent. Preview deployments remain email-silent, and manual workflow runs default to validation without delivery.
