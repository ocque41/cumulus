...
- [x] Jacquard `h1`–`h6` heading families remain unchanged; bundled OFL-licensed GFS Neohellenic covers every non-heading role, including body, navigation, controls, labels, metadata, large links, interlude copy, project names, and footer statements. Live Vercel Preview and Production configuration remain separate external gates.
...
- [x] The historical baseline introduced no other typography family. The current GFS Neohellenic non-heading role is governed by the candidate delta and OFL boundary above.
