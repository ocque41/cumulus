...
| R-05 | Literal branch follows the supplied reference's visual structure without copying third-party copy or media | Responsive comparison evidence and content review | In progress | The literal branch retains original Cumulus copy/media and the reference hierarchy; renewed browser proof is required after the final shared-change reconciliation |
| R-06 | A cloned branch provides an independently designed visual structure only | Branch graph, visual-only scope diff, full verification | In progress | `c9d2b49` records the initial visual-only delta from the first complete reference implementation; final shared-change reconciliation and scope diff remain before closure |
...
| R-20 | Both designs are responsive, accessible, reduced-motion safe, and usable without WebGL/provider availability | Complete design checklist, automated accessibility checks, fallback walkthroughs | In progress | Selected branch passes desktop/mobile E2E and primitive tests cover reduced motion, WebGL failure, fallbacks, focus, and dialog behavior; the reconciled reference branch still needs its renewed browser pass |
| R-21 | Lint, typecheck, unit, build, security, license, and end-to-end checks pass from a clean install | Current unabridged command outputs and coverage review | In progress | Selected candidate passes lint, dual TypeScript compilation, 142 unit tests, build with 26 static routes, public-safety scan, license check, zero-vulnerability audit, and 9 browser tests with one intentional desktop skip; reference-branch rerun remains |
| R-22 | Live domain serves the selected proven commit and production notification flow works | Production URL checks, commit linkage, controlled live lifecycle | In progress | The live domain serves the previously selected release; the current candidate still needs deployment verification, and notification publishing remains fail-closed pending truthful sender/legal configuration and controlled lifecycle evidence |
...
| `E-002` | `R-05,R-08,R-09,R-20,R-21` | `request/jacquard-reference@8ba7ccc` | Clean install; `npm run verify`; `npm run test:e2e`; rendered checks at 375/768/1280/1600 px | `2026-07-16T16:04:35Z` | Pass | Reference branch and this matrix | Primary agent |
| `E-003` | `R-06,R-07,R-10,R-13,R-14,R-20,R-21` | selected release candidate | Lint; dual typecheck; 142 unit tests; build; 9 browser tests with one desktop-only skip | `2026-07-16` | Pass on selected candidate | Production branch and this matrix | Primary agent |
| `E-004` | `R-04,R-18,R-22` | current production main | Redacted Vercel inspection; production deploy `READY`; alias and environment-name review | `2026-07-16` | Partial | `https://cumulush.com` | Primary agent |
...
| `E-006` | `R-15,R-16,R-17,R-22,R-23` | current production main | Auth/notification tests; live fail-closed publish probe | `2026-07-16` | Partial | Missing truthful postal address, webhook secret, and controlled production delivery evidence | Primary agent |
